...
// Exercises: Option and Result usage, parsing errors, custom validation returning Result, using ? operator, and reading files with Result propagation.
// Entry: uses std::fs and demonstrates error handling patterns
use std::fs;
use std::num::ParseIntError;

pub fn run() {
    // Exercise 10-1: Option usage and highest score printing
    let scores = vec![92, 81, 77, 100, 68];
    let empty_scores: Vec<i64> = vec![];
    print_highest_score(&scores);
    print_highest_score(&empty_scores);

    // Exercise 10-2: parsing integers and handling ParseIntError
    ...
    // Exercise 10-3: composing Results and using the ? operator in helper functions
